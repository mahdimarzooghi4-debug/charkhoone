...
output_dir="${1:-artifacts/base-image-drift}"
current_lock="deploy/base-images.lock"
resolved_lock="${output_dir}/resolved-base-images.lock"
...
if ! bash scripts/release/resolve-base-image-digests.sh "$resolved_lock" >"$resolver_stdout" 2>"$resolver_stderr"; then
...
EOF
  echo "Base-image registry resolution failed; see sanitized resolver evidence." >&2
  exit 2
fi
...
echo "Base-image digest drift detected. Review evidence before updating any pin." >&2
exit 1
