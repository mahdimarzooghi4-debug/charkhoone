...
- The `/api/v1` route group requires authorization by default. This prevents a future endpoint from becoming anonymous merely because its mapping forgot `.RequireAuthorization()`.
...
- development-only external adapter mocks remain rejected outside `Development`.
