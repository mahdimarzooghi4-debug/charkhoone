...
public sealed class PaymentPersistenceModelTests
{
    ...
    public void MonthlyObligation_IsUniquePerContractMonth()
    {
        ...
        Assert.Contains(entity.GetIndexes(), index => index.IsUnique && index.Properties.Select(x => x.Name).SequenceEqual([nameof(MonthlyObligationRow.ContractId), nameof(MonthlyObligationRow.ContractMonthNumber)]));
    }
    ...
    public void MonthlyObligationComponent_IsUniquePerKindAndInstruction()
    {
        ...
        Assert.Contains(entity.GetIndexes(), index => index.IsUnique && index.Properties.Select(x => x.Name).SequenceEqual([nameof(MonthlyObligationComponentRow.MonthlyObligationId), nameof(MonthlyObligationComponentRow.Kind)]));
    }
    ...
    public void ContractDelinquency_DoesNotReferenceFrozenPrincipal()
    {
        ...
        Assert.DoesNotContain(entity.GetProperties(), property => property.Name.Contains("FrozenPrincipal", StringComparison.OrdinalIgnoreCase));
    }
    ...
    public void PaymentInstruction_RemainsLosslessRialAndBelongsToMonthlyObligation()
    {
        ...
        Assert.Equal("numeric", amount!.GetColumnType());
        Assert.Null(amount.GetPrecision());
        Assert.Null(amount.GetScale());
        Assert.Contains(entity.GetForeignKeys(), fk => fk.PrincipalEntityType.ClrType == typeof(MonthlyObligationRow) && fk.Properties.Single().Name == nameof(PaymentInstructionRow.ObligationId));
    }

    private static CharkhooneDbContext CreateContext()
    {
        var options = new DbContextOptionsBuilder<CharkhooneDbContext>().UseNpgsql("Host=localhost;Database=charkhoone_model_test;Username=test;Password=test").Options;
