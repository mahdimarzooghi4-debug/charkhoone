    ...
    public void Configure(EntityTypeBuilder<PaymentInstructionRow> builder)
    {
        builder.ToTable("payment_instructions", table =>
        {
            table.HasCheckConstraint(
                "CK_payment_instructions_positive_finite_amount",
                "\"AmountRial\"::text NOT IN ('NaN','Infinity','-Infinity') AND \"AmountRial\" > 0");
            table.HasCheckConstraint(
...
public sealed class FrozenPrincipalRowConfiguration : IEntityTypeConfiguration<FrozenPrincipalRow>
{
    public void Configure(EntityTypeBuilder<FrozenPrincipalRow> builder)
    {
        ...
        {
            table.HasCheckConstraint(
                "CK_frozen_principals_positive_finite_amount",
                "\"AmountRial\"::text NOT IN ('NaN','Infinity','-Infinity') AND \"AmountRial\" > 0");
            table.HasCheckConstraint(
                "CK_frozen_principals_bank_id_nonblank",
                "btrim(\"BankId\") <> ''");
            table.HasCheckConstraint(
                "CK_frozen_principals_fund_reference_nonblank",
                "btrim(\"FundReference\") <> ''");
        });
