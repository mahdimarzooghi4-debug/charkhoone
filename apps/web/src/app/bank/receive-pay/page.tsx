...
function FundingAction({ request, name, amount, href }: { request:string; name:string; amount:string; href:string }) {
  return <div className={styles.actionRow}><div className={styles.actionTop}><span className={styles.caseInfo}><strong>{request}</strong><small>{name}</small></span><span className={styles.actionType}>تأمین اصل تسهیلات</span><span className={styles.actionAmount}>{amount}</span><span className={styles.readyBadge}>آماده تأمین</span><Link href={href} className={styles.primaryButton}>تأمین وجه</Link></div><div className={styles.checks}><span className={styles.check}><b>✓</b><span>وجه در کارگزاری همین بانک تأیید شده</span></span><span className={styles.check}><b>✓</b><span>درخواست بانک تأیید شده</span></span><span className={styles.check}><b>✓</b><span>وجه مستأجر واریز شده</span></span></div></div>;
}
