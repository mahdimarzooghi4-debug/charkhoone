...
export default function OrganizationPlanActivationPage() {
  return (
    <section className="org-plan-activation" data-node-id="480:19">
      <header className="org-plan-flow__header">
        ...
        <div>
          <h1>انتخاب پرسنل و فعال‌سازی طرح</h1>
      ...
      <article className="org-plan-card org-plan-activation__summary">
        <div className="org-plan-detail__section-head">
          <span className="org-plan-status org-plan-status--active">قابل فعال‌سازی</span>
          <div><h2>خلاصه طرح انتخاب‌شده</h2></div>
        </div>
        <div className="org-plan-activation__summary-grid">
          {summary.map((item) => (
            <div key={item.label}>
      ...
      <div className="org-plan-activation__controls">
        <div>
          <button type="button" className="org-bank-plans__filter">فقط واجد شرایط</button>
          <button type="button" className="org-bank-plans__filter">همه واحدها</button>
        ...
        <table className="org-plan-selection-table">
          ...
          <tbody>
            {rows.map((row) => {
              ...
              return (
                <tr className={disabled ? "org-plan-selection-table__disabled" : ""} key={row.employeeCode}>
                  <td>
                    <input
                      className="org-plan-selection-checkbox"
                      type="checkbox"
      ...
      <article className="org-plan-activation__selection-summary">
        <div>
          <strong>۶ نفر انتخاب شده‌اند</strong>
          <span>افراد نیازمند تکمیل یا دارای طرح فعال انتخاب نمی‌شوند.</span>
        </div>
        <div>
          <strong>بعد از فعال‌سازی چه می‌شود؟</strong>
          <span>طرح برای افراد انتخاب‌شده فعال می‌شود و تعهدهای پرداخت سازمان طبق شرایط همین طرح ساخته می‌شود.</span>
        </div>
      </article>

      <footer className="org-plan-flow__actions">
        <Link href="/organization/bank-plans/supportive-employees/activate/review" className="org-action-button org-action-button--primary">ادامه و بررسی فعال‌سازی</Link>
        <Link href="/organization/bank-plans/supportive-employees" className="org-action-button org-action-button--surface">انصراف</Link>
      </footer>
    </section>
  );
}
