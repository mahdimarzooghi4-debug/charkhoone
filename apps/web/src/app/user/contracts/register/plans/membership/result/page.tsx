...
export default async function MembershipPaymentPreviewPage({
  ...
  searchParams: Promise<{ plan?: string; membership?: string }>;
}) {
  // Display the selected fixture only; this is not a payment result from a provider.
  const { plan, membership } = await searchParams;
  const financingPlan = plan === "general" ? "general" : "staff";
  const chosenId = membership === "twice" || membership === "twice-high" ? membership : "once";
    ...
    <main className={styles.page} data-node-id="175:754" data-name="Web App / Payment Return / Membership Success">
      <section className={styles.mainContent} data-node-id="175:755">
        <header className={styles.headerBlock} data-node-id="175:756">
          <Link href={`/user/contracts/register/plans/membership?plan=${financingPlan}`} className={styles.backLink} data-node-id="175:757">بازگشت به انتخاب عضویت <span>›</span></Link>
