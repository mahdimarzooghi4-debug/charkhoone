...
function previewCaseDetail(item: PilotCaseQueueItem): PilotCaseDetail {
  ...
  return {
    ...
    verificationRequests: [
      { id: `verification-${item.creditApplicationId.slice(0, 8)}`, type: item.latestVerificationType ?? "Identity", provider: "PreviewProvider", status: item.latestVerificationStatus ?? "Verified", externalReference: "preview-reference", reasonCode: null, attemptCount: 1, updatedAtUtc: now },
    ],
    creditEligibility: item.creditEligibilityStatus ? { provider: "CreditPreview", status: item.creditEligibilityStatus, externalSubGrade: "A2", fullDepositEquivalentRial: "12500000000", loanRatio: "0.35", maximumEligibleLoanRial: "4375000000", externalReference: "credit-preview", reasonCode: null, attemptCount: 1, updatedAtUtc: now } : null,
    bankApproval: item.bankApprovalStatus ? { provider: "BankSample", status: item.bankApprovalStatus, maximumEligibleLoanRial: "4375000000", approvedLoanRial: item.bankApprovalStatus === "Approved" ? "3500000000" : null, externalReference: "bank-preview", reasonCode: null, attemptCount: 1, updatedAtUtc: now } : null,
    ...
    fundFreeze: item.fundFreezeStatus ? { provider: "BrokeragePreview", status: item.fundFreezeStatus, fundReference: "fund-preview-01", externalReference: "freeze-preview", reasonCode: null, attemptCount: 1, updatedAtUtc: now } : null,
    tenantContributionFunding: item.tenantContributionStatus ? { fundingId: `funding-${item.creditApplicationId.slice(0, 8)}`, externalTransactionId: `trx-${item.creditApplicationId.slice(0, 8)}`, provider: "PaymentPreview", status: item.tenantContributionStatus, amountRial: "9000000000", currency: "IRR", fundReference: "fund-preview-01", externalReference: "tenant-preview", reasonCode: null, attemptCount: 1, updatedAtUtc: now } : null,
    recentAuditEvents: [
      { id: `audit-${item.creditApplicationId.slice(0, 8)}-1`, actorId: "preview-operator", action: "CaseViewed", reason: "Stage preview fixture", occurredAtUtc: now },
      { id: `audit-${item.creditApplicationId.slice(0, 8)}-2`, actorId: "system", action: "StateUpdated", reason: "Preview lifecycle event", occurredAtUtc: "2026-09-23T09:00:00Z" },
    ],
