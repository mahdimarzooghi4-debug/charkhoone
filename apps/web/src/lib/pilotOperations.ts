...
function previewCaseDetail(item: PilotCaseQueueItem): PilotCaseDetail {
  ...
  return {
    ...
    bankLoanPlanId: "طرح مسکن ۱۴۰۵",
    bankLoanPlanVersion: "v1",
    contractId: item.contractId,
    contractStatus: item.contractStatus,
    ownerUserId: item.contractId ? "مالک آزمایشی ۰۱" : null,
    propertyId: item.contractId ? "ملک آزمایشی ۰۱" : null,
    verificationRequests: [
      { id: `verification-${item.creditApplicationId.slice(0, 8)}`, type: item.latestVerificationType ?? "Identity", provider: "سرویس آزمایشی", status: item.latestVerificationStatus ?? "Verified", externalReference: "مرجع آزمایشی", reasonCode: null, attemptCount: 1, updatedAtUtc: now },
    ],
    creditEligibility: item.creditEligibilityStatus ? { provider: "سامانه اعتبارسنجی", status: item.creditEligibilityStatus, externalSubGrade: "A2", fullDepositEquivalentRial: "12500000000", loanRatio: "0.35", maximumEligibleLoanRial: "4375000000", externalReference: "مرجع اعتبارسنجی آزمایشی", reasonCode: null, attemptCount: 1, updatedAtUtc: now } : null,
    bankApproval: item.bankApprovalStatus ? { provider: "بانک نمونه", status: item.bankApprovalStatus, maximumEligibleLoanRial: "4375000000", approvedLoanRial: item.bankApprovalStatus === "Approved" ? "3500000000" : null, externalReference: "مرجع بانکی آزمایشی", reasonCode: null, attemptCount: 1, updatedAtUtc: now } : null,
    fundingAllocation: item.contractId ? { id: `allocation-${item.creditApplicationId.slice(0, 8)}`, contractId: item.contractId, bankId: "بانک نمونه", fullDepositEquivalentRial: "12500000000", maximumEligibleLoanRial: "4375000000", bankApprovedLoanRial: "3500000000", tenantContributionRial: "9000000000", updatedAtUtc: now } : null,
    fundFreeze: item.fundFreezeStatus ? { provider: "کارگزاری نمونه", status: item.fundFreezeStatus, fundReference: "صندوق آزمایشی ۰۱", externalReference: "مرجع تثبیت آزمایشی", reasonCode: null, attemptCount: 1, updatedAtUtc: now } : null,
    tenantContributionFunding: item.tenantContributionStatus ? { fundingId: `funding-${item.creditApplicationId.slice(0, 8)}`, externalTransactionId: `trx-${item.creditApplicationId.slice(0, 8)}`, provider: "شبکه پرداخت", status: item.tenantContributionStatus, amountRial: "9000000000", currency: "IRR", fundReference: "صندوق آزمایشی ۰۱", externalReference: "مرجع پرداخت آزمایشی", reasonCode: null, attemptCount: 1, updatedAtUtc: now } : null,
    recentAuditEvents: [
      ...
      { id: `audit-${item.creditApplicationId.slice(0, 8)}-2`, actorId: "سامانه", action: "StateUpdated", reason: "رویداد چرخه آزمایشی", occurredAtUtc: "2026-09-23T09:00:00Z" },
    ],
