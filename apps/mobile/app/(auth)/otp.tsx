  ...
  return (
    <AuthScaffold>
      ...
      <View style={styles.mobileRow}>
        <Pressable onPress={() => router.back()}><Text style={styles.edit}>ویرایش شماره موبایل</Text></Pressable>
        <Text style={styles.mobile}>۰۹۱۲۱۲۳۴۵۶۷</Text>
      </View>
      <Pressable onPress={() => inputRef.current?.focus()} style={styles.otpRow}>
        {digits.map((digit, index) => (
          <View key={index} style={styles.digit}><Text style={styles.digitText}>{digit}</Text></View>
        ))}
      </Pressable>
      <TextInput
        ref={inputRef}
